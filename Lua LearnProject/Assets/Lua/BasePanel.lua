...
function BasePanel:Init(name)
    if self.panelObj == nil then
        --公共的实例化对象的方法
        self.panelObj = ABMgr:LoadRes("ui", name, typeof(GameObject))  ---------------加载UI预制体
        self.panelObj.transform:SetParent(Canvas, false) --------------------------设置父级（位置）

        
        --GetComponentsInChildren
        --找到所有UI控件 存起来
        print(self.panelObj)
        local allControls = self.panelObj:GetComponentsInChildren(typeof(UIBehaviour)) -----------得到其所有控件
        --如果存入一些对于我们来说没用UI控件 
        ...
        --Image img名字
        --ScrollRect sv名字
        for i = 0, allControls.Length-1 do  ---------------------------------将其得到的所有控件 按一定存放形式保存到一个元表中
            local controlName = allControls[i].name   --组件的名字（世界里的物体GameObject)
            ...
            if string.find(controlName, "btn") ~= nil or 
               ...
               string.find(controlName, "txt") ~= nil then
                ...
                --利用反射 Type 得到 控件的类名 
                local typeName = allControls[i]:GetType().Name  --组件绑定的控件的类名
                --避免出现一个对象上 挂在多个UI控件 出现覆盖的问题 
                --都会被存到一个容器中 相当于像列表数组的形式
                --最终存储形式 
                --{ btnRole（控件的命名） = { Image（控件的类型） = （具体控件）, Button （控件的类型）= （具体控件） },
                --  togItem（控件的命名） = { Toggle = 控件}  }
